...
**The attack test** — not a unit stub; the agent actually appends a `vote` op **over a real Corestore replication stream** (the same wire protocol Hyperswarm rides on; the live DHT/swarm path is exercised separately by `create`/`join`/`npm run verify:p2p --swarm`) and every honest peer rejects it:
...
**Honest caveat on this claim**: agent-inferiority is enforced *by role*, and role comes from whichever `add-writer` op granted the seat — so it depends on the grantor tagging it correctly. A human who runs `/approve <key> human` on a writer that self-declared `agent` hands it full human authority; the CLI now warns on that specific override but does not block it. Full threat model, including the solo-self-Sybil consensus risk, in [AUDIT_REPORT.md §3](docs/AUDIT_REPORT.md#3-residual-risks-deliberate-documented).
...
>
> **Proof, not just a promise:** an unedited capture of exactly this run (real Qwen3 rationale, real autonomous stake, machine won) is in the [README's "See it in action"](README.md#-see-it-in-action) section — every runnable surface (demo/e2e/verifiers/Pear UI) defaults to the heuristic brain for offline speed, so this transcript is the evidence that the QVAC path isn't just wired, it actually runs.
